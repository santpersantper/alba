// components/community-settings/MyEvents.js
// DROP-IN: Settings now navigates to EventSettings with correct params (postId).
  ...
  return (
    <View style={[styles.card, { backgroundColor: cardBg, borderColor: border }]}>
      <View
        style={[
          styles.iconWrap,
          { backgroundColor: isDark ? "#2a1f22" : "#ffe4e4" },
        ]}
      >
        <View
          style={[
            styles.iconDot,
            { backgroundColor: isDark ? "#2bb9d6" : "#56d1f0" },
          ]}
        />
      </View>
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.rowBtns}>
          <TouchableOpacity
            style={[styles.pill, { backgroundColor: "#3DD3F2" }]}
            onPress={onPressChat}
            activeOpacity={0.85}
          >
            <Image
      ...
      {loading ? (
        ...
        rows.map((row) => (
          <EventCard
            ...
            onPressSettings={() => {
              // ✅ NEW: go to EventSettings with correct param (postId)
              navigation?.navigate?.("EventSettings", { postId: row.id });
            }}
          />
        ))
      )}
